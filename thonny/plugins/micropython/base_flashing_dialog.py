...
class BaseFlashingDialog(WorkDialog, ABC):
    ...
    def _download_variants(self):
        ...
        try:
            url = self.get_variants_url()
            parsed_url = urlparse(url)
            print("#######", parsed_url,parsed_url.scheme)
            if parsed_url.scheme == 'file':
                # Handle local file URLs
                with open(parsed_url.path, 'r', encoding="UTF-8") as fp:
                    json_str = fp.read()
            else:
            ...
            #logger.info("Got %r variants", variants)
            

        except Exception as e:
            msg = f"Could not download variants info from {url} and exception is {e}"
