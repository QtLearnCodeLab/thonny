...
class ESPFlashingDialog(BaseFlashingDialog):
    ...
    def get_families_mapping(self) -> Dict[str, str]:
        codes = ["esp32","esp32-data","esp32s2", "esp32s3", "esp32c3"]
    ...
    def populate_main_frame(self):
        ...
        x61000_target_description = "for MicroPython on QtPi ESP32 16MB data partition"
        address_mapping = {
            f"0x0 ({x0_target_description})": "0x0",
            f"0x1000 ({x1000_target_description})": "0x1000",
            f"0x610000 ({x61000_target_description})": "0x610000",
        }
        ...
        self._speed_combo.select_value("460800")
        ...
        # Find the index corresponding to the default value key
        default_index = list(flash_size_mapping.keys()).index(default_flash_size_key)

        # Set the default value in the combobox
        self._flash_size_combo.current(default_index)
    ...
    def get_variants_url(self) -> str:
        return f"https://qtlearncodelab.github.io/qtpi-shared/firmware/veda/v3/{self.firmware_name.lower()}-variants-esptool.json"
    

    def get_variants_url_local(self) -> str:
        # Get the directory where your package is installed
        plugin_dir = os.path.dirname(__file__)

        # Go up two levels to reach the grandparent directory where 'data' is located
        parent_dir = os.path.dirname(os.path.dirname(plugin_dir))
        # Construct the absolute path to your JSON file (assuming it's in a 'data' folder)
        local_file_path = os.path.join(parent_dir, "data", f"{self.firmware_name.lower()}-variants-esptool.json")

        # Convert the local path to a URL
        return urllib.parse.urljoin('file:', urllib.parse.pathname2url(local_file_path))
    ...
    def perform_core_operation(
        ...
        if self._work_mode == "install":
            ...
            if variant_info["family"] not in ["esp32-data"]:
                command.append("--baud")
                command.append(work_options["speed"])
                command.append("--chip")
                command.append(work_options["family"]) 
            ...
            '''
            command = self._esptool_command + [
                "--port", target_info.port.device
            ]

            if variant_info["family"] not in ["esp32-data"]:
                command += [
                    "--baud", work_options["speed"],
                    "--chip", work_options["family"],
                    "write_flash",
                    "--flash_mode", work_options["flash_mode"],
                    "--flash_size", work_options["flash_size"]
                ]
                if work_options["no_stub"]:
                    command.append("--no-stub")
                if work_options["erase_flash"]:
            ...
            command += [work_options["address"], source_path]
    ...
    def _compute_start_address(self, family: str) -> str:
        if self.firmware_name == "MicroPython" and family in ["esp32", "esp32s2"]:
            return "0x1000"
        elif self.firmware_name == "MicroPython" and family in ["esp32-data"]:
            return "0x610000"
        else:
            return "0x0"
